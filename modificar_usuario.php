...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["user"]) && isset($_POST["pass"]) && isset($_POST["email"]) && isset($_POST["plan"])) {
        $servidor = "localhost";
        ...
        if (!empty($nuevoUsuario) && !empty($nuevaContrasena) && !empty($nuevoCorreo) && !empty($nuevoPlan)) {
            $conexion = mysqli_connect($servidor, $usuariodb, $passdb, $db);
            if (!$conexion) {
                die("Error de conexión: " . mysqli_connect_error());
            }

            $consulta_verificar = "SELECT * FROM usuarios WHERE (nombre=? OR correo_electronico=? OR plan=?) AND nombre != ?";
            $stmt_verificar = mysqli_prepare($conexion, $consulta_verificar);
            mysqli_stmt_bind_param($stmt_verificar, "ssss", $nuevoUsuario, $nuevoCorreo, $nuevoPlan, $usuarioActual);
            mysqli_stmt_execute($stmt_verificar);
            mysqli_stmt_store_result($stmt_verificar);

            if (mysqli_stmt_num_rows($stmt_verificar) == 0) {
                $consulta_actualizar = "UPDATE usuarios SET nombre=?, correo_electronico=?, contrasena=?, plan=? WHERE nombre=?";
                $stmt = mysqli_prepare($conexion, $consulta_actualizar);
                $hashed_password = password_hash($nuevaContrasena, PASSWORD_DEFAULT);
                mysqli_stmt_bind_param($stmt, "sssss", $nuevoUsuario, $nuevoCorreo, $hashed_password, $nuevoPlan, $usuarioActual);
                ...
                echo "Error: El nombre de usuario, correo electrónico o plan ya están en uso.";
            }
